...
const actions = {
	async getAll({commit}) {
		let res = await axios.get('mural')
		commit('setList', res.data)
	},
	async getOne({commit }, id) {
		let res = await axios.get(`mural/${id}`)
		commit('SET_ITEM', res.data)
	},
	async update({commit}, data) {
		commit('UPDATE_ITEM_FROM_LIST', data)
		let res = await axios.put(`mural/${data.id}`, data)
	},
	async store({commit}, data) {
		commit('ADD_ITEM_FROM_LIST', data)
		let res = await axios.post('mural', data)
	},
	async destroy({commit}, id) {
		commit('REMOVE_FROM_LIST', id)
		let res = await axios.delete(`mural/${id}`)
	},
}
